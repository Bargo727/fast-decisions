...
N = 10.^[1 2 3];                         % Number and range of agents
J = 1;                                   % jth quickest agent
th = 1;                                  % Interval half length
WW = th.*linspace(-.99,.99,500);
...
figure;
for i = 1:length(N)
    % f = fit(WW',(bdy(:,i)./prob(:,i)),'logistic')
    % plot(f,WW',(bdy(:,i)./prob(:,i)));
...
xlim([-th th]);
y = ylabel('Prob($n(1)$ makes correct decision$\,| \,X_{n(1)}(0) = y$)');
